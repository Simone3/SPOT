...
- `src/main/ipc/DiagnosticsIpc.ts` is the renderer's one way into the operational log: it writes the task state drift the audit reports and answers with the log file it wrote it to.
...
`spotDiagnostics` carries one operation, `reportTaskStateDrift()`, and it is deliberately not part of `spotStorage`: it stores nothing and reads nothing, and what it hands over is only useful to someone reading the log file afterwards. The main process decides what is written and at which level, and it writes no more differences than the audit itself reports, so the channel can never put the whole task list on one log line. It answers with the path of the log file it wrote to, or with nothing when logging is unavailable, which is the only way the renderer can name that file to the user.
...
It also logs the one thing the renderer asks it to: a task state drift, as a `task.audit` warning holding both task counts, the real difference count, and the capped list of differences the audit reported. That is the only renderer-written entry, and the renderer chooses neither its message nor its level.
