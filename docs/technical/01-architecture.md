...
| `window.spotBackupLocation` | `src/types/BackupLocationIpcChannels.ts`, `BackupLocationTypes.ts`, `src/main/ipc/BackupLocationIpc.ts` | Where the backups go and how many are kept: reading the settings, the native folder dialog, choosing a folder, and choosing the number of copies ([§6.5](06-persistence.md#65-choosing-the-backup-folder-and-the-number-of-copies)) |
