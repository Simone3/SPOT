...
- **A control with no room around its own content stands the ring off from it.** `TextArea` is the one that needs it, because the text fills the field and the card gives it no padding to spare: the ring is drawn by the container on a pseudo-element inset outwards from it, which stands off from the text without moving anything else on the task card.
