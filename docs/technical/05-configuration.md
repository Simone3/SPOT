...
| `BACKUP_CONFIG` | The default backup directory name, the backup file prefix and extension, the name of the copy kept up to date, the partial and temporary file names used while a copy is being written, the delay after the last task change before that copy is refreshed, how old the newest dated copy has to be before another is taken and how often that is checked, the default number of copies kept and the range a user's choice is held to, and the bounded time the shutdown backup is given |
