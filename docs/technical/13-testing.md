...
- **Storage and the main process** (`tests/main`, the framework storage and backup tests) cover SQLite setup, row mapping, one transaction per command and its rollback, the lifecycle of both kinds of backup copy including one that reopens as a valid database, and the scheduler's timing and retention rules.
...
- **Render through the factories in `tests/testUtils`.** Components that show text need `TranslationProvider` above them, so component tests render through `renderWithTranslations()`. It passes the provider as the Testing Library `wrapper` rather than wrapping the element, so `rerender` keeps it in place. `makeTranslator()` in the same helper supplies the English translator, for a test that words something itself.
