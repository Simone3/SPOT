...
- Electron loads `http://localhost:3000` in development and the built React `build/index.html` file in packaged mode. `npm run package` and `npm run make` run `npm run build-react` first, and CRA uses `homepage: "."` so packaged assets resolve from the file-loaded build.
