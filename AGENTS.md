...
- Runtime persistence is wired for Electron startup and task mutations. Electron registers storage IPC handlers through `src/main/ipc/TaskStorageIpc.ts`, the bundled preload from `src/main/preload/Preload.ts` exposes them as `window.spotStorage`, and React requires that API for task loading and mutations. Browser-only sample-data/local mutation mode is no longer supported.
